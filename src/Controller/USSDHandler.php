...
class USSDHandler
{
    ...
    public function control()
    {
        ...
        if (!isset($this->sessionId) || !isset($this->serviceCode) || !isset($phoneNumber) || !isset($ussdBody) || !isset($networkCode)) {
            ...
            $this->msgType = "2";
        } else if (empty($this->sessionId) || empty($this->serviceCode) || empty($phoneNumber) || empty($ussdBody) || empty($networkCode)) {
    ...
    private function mainMenuResponse()
    {
        ...
        $this->totalFormsAvailable  = count($allForms);
    ...
    private function setSessionLevels()
    {
        if (!isset($_SESSION["level"][0])) {
            $input = (int) $this->ussdBody;
            if ($input >= 1 && $input <= $this->totalFormsAvailable) {
                $_SESSION["level"][1] = $this->ussdBody;
            }
        } elseif (isset($_SESSION["level"][0]) && !$_SESSION["level"][1]) {
            $input = (int) $this->ussdBody;
            if ($input == 1) {
                $_SESSION["level"][2] = $this->ussdBody;
            }
        } elseif (isset($_SESSION["level"][1]) && !$_SESSION["level"][2]) {
            if ($this->validateTextInputs($this->ussdBody)) {
                $_SESSION["level"][3] = $this->ussdBody;
            }
        } elseif (isset($_SESSION["level"][2]) && !$_SESSION["level"][3]) {
            if ($this->validateTextInputs($this->ussdBody)) {
                $_SESSION["level"][4] = $this->ussdBody;
            }
        } elseif (isset($_SESSION["level"][3]) && !$_SESSION["level"][4]) {
            if ($this->validateIntegerInputs($this->ussdBody)) {
                $_SESSION["level"][5] = $this->ussdBody;
            }
        }
    }
    ...
    private function continueResponse()
    {
        ...
        if (isset($_SESSION["level"][0]) && !$_SESSION["level"][1]) {
            ...
            $response = $formInfo[0]["name"] . " forms cost GHc " . $formInfo[0]["amount"] . ".  Enter 1 to continue {$_SESSION["level"][0]}.\n";
            $response .= "1. Buy";
        }
        //
        elseif (isset($_SESSION["level"][1]) && !$_SESSION["level"][2]) {
            $this->msgType = '1';
            $response = "Enter your first name. {$_SESSION["level"][1]}";
        }
        //
        else if (isset($_SESSION["level"][2]) && !$_SESSION["level"][3]) {
            $this->msgType = '1';
            $response = "Enter your last name. {$_SESSION["level"][2]}";
        }
        //
        else if (isset($_SESSION["level"][3]) && !$_SESSION["level"][4]) {
            $this->msgType = '1';
            $response = "Enter the Mobile Money number to buy the form. {$_SESSION["level"][3]} eg 024XXXXXXX";
        }
        //
    ...
    private function fetchSessionLogs($sessionID, $phoneNumber)
    {
        $query = "SELECT * INTO `ussd_activity_logs` WHERE ";
        $params = array(":r" => $request);
        $this->dm->inputData($query, $params);
    }
}
